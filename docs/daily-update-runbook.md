...
>
> **Energy/LNG ingestion**: See [`energy-lng-data-ingestion.md`](energy-lng-data-ingestion.md) for the active staged connector, provenance and promotion contract. `bun run energy:refresh` produces a fingerprinted candidate/report pair; `bun run daily:update` promotes only a complete, validated same-as-of report whose fingerprint still matches current inputs.
...
| **TTF** | 63.92 EUR/MWh (2026-08-18, Yahoo Finance TTF=F; +5.82%); carried after machine refresh failure |
...
**The running app fetches every JSON dataset from `public/data/*.json` at runtime (`useStaticJson`), not from `src/data/`.** `bun run daily:apply` auto-mirrors `daily-state.json`, `exposure-traces.json`, the Energy/LNG runtime/read model, `iran-intel-events.json`, `banker-cross-asset.json`, `banker-conflict.json`, `charts-volatility.json`, and `commodities-impact.json`. `bun run daily:check` rejects drift for all of these.
...
- [ ] **Stage Energy/LNG refresh**: Run `bun run energy:refresh` after the liquid refresh and after any state, exposure, evidence, or registry edit. For event-driven changes use `bun run energy:refresh -- --event`; review selector-scoped snapshots, machine evidence, carry/unavailable outcomes, and the fingerprinted candidate/report pair.
